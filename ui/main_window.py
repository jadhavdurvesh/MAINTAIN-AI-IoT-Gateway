...
from PySide6.QtWidgets import QApplication, QComboBox, QFormLayout, QFrame, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMessageBox, QPushButton, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def build_ui(self):
        ...
        outer.addWidget(QLabel("IoT Gateway  •  Wired machine connectivity"))

        connection = QFrame()
        connection.setObjectName("card")
        form = QFormLayout(connection)
        form.setContentsMargins(22, 18, 22, 18)
        self.port = QComboBox(); self.port.setMinimumWidth(300)
        ...
        self.machine = QLineEdit(); self.machine.setPlaceholderText("Machine name / label")
        ...
        actions = QHBoxLayout()
        self.refresh = QPushButton("Refresh Ports"); self.connect_btn = QPushButton("Connect Arduino"); self.test_btn = QPushButton("Test Backend"); self.disconnect_btn = QPushButton("Disconnect")
        for button in (self.refresh, self.connect_btn, self.test_btn, self.disconnect_btn): actions.addWidget(button)
        ...
        for row, label, value in [(0, "Arduino", self.serial_status), (1, "Backend", self.backend_status), (2, "Machine", self.machine_status), (3, "Last upload", self.last_status)]: status_grid.addWidget(QLabel(label), row, 0); status_grid.addWidget(value, row, 1)
        ...
        self.log = QLabel("Ready. Connect an Arduino sending one JSON object per line.")
    ...
    def refresh_ports(self):
        ...
        for device in devices: self.port.addItem(f"{device.port} — {device.description}", device.port)
        if current:
    ...
    def connect_serial(self):
        ...
        self.manager.baud_rate = int(self.baud.currentText()); self.manager.set_device_key(self.key.text()); self.manager.api_url = self.api.text().strip()
        if not self.manager.device_key: return QMessageBox.warning(self, "Device key", "Enter the machine IoT device key first.")
        try:
            self.manager.serial.connect(self.port.currentData(), self.signals.readings.emit, self.signals.error.emit)
            self.serial_status.setText("● Connected"); self.machine_status.setText(self.machine.text().strip() or "Paired device"); self.log.setText("Arduino connected. Waiting for sensor data…"); self.save()
    ...
    def test_backend(self):
        ...
        ok, message = self.manager.test_backend(); self.backend_status.setText("● Connected" if ok else "● Unavailable"); self.log.setText(message); self.save()
    ...
    def handle_readings(self, payload):
        ...
        for name, value, unit in readings:
            if name in self.sensor_labels: self.sensor_labels[name].setText(f"{value:g} {unit}")
            ok, status, message = self.manager.api().send(name, value, unit)
            if not ok: failures.append(f"{name}: {status or message}")
        if failures:
            self.backend_status.setText("● Unavailable"); self.log.setText("Upload failed — " + "; ".join(failures))
        else:
